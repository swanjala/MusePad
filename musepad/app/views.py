...
from flask import abort, jsonity, request
...
class RegisterUser(Resource):
	...
	def __init__(self):
		self.reqparse = required.RequestParser()
...
class MuseListAction(Resource):
	...
	def get(self, id=None):
		...
		if search:
			muse_search_results= Muselist.query.filer(Muselist.name.ilike(
		...
		if page or limit:
			...
			muse_display = [
				muse_dis for muse_disp in muselist_collection.items]
				return muse_display
...
class ItemAction(Resource):
	...
	def post(self, id=None):
		...
		item = Item(name=name, muse_id=id, item_description=item_description)
